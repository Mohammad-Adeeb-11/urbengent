    ...
    const loadSummary = async () => {
      try {
        ...
        setSummary({
          ...
          wishlist: (wishlist.data.products || []).length,
  ...
  const cards = [
    {
      ...
      path: "/account/orders",
      icon: Package,
      color: "bg-[#eef3fb] text-[#315687]",
    },
    {
      label: "Wishlist",
      value: summary.wishlist,
      detail: "Pieces saved for later",
      path: "/account/wishlist",
      icon: Heart,
      color: "bg-[#fff2ef] text-[#b45d42]",
    },
    {
      label: "Addresses",
    ...
    <div>
      <div className="border-b border-slate-100 pb-6">
        ...
        <p className="mt-1 text-sm text-slate-500">
          Everything important about your UrbanGent account, in one place.
        </p>
      </div>
      <div className="mt-7 grid gap-4 sm:grid-cols-3">
        {cards.map(({ label, value, detail, path, icon: Icon, color }) => (
          <Link
            key={path}
            to={path}
          ...
          >
            ...
            <p className="mt-5 text-2xl font-semibold text-[#16283f]">
              {loading ? "--" : value}
            </p>
      ...
      <div className="mt-7 rounded-xl bg-[#f8fafb] p-5 sm:p-6">
        <div className="flex items-center gap-3">
          <span className="rounded-lg bg-[#e9b872] p-2 text-[#16283f]">
            <UserRound size={18} />
          </span>
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              A current profile helps us make every order smoother.
            </p>
          </div>
        </div>
        <Link
          to="/account/profile"
          className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-[#b77a2e] hover:text-[#16283f]"
        >
